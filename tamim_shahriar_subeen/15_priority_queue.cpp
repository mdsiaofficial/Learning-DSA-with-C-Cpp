...
int insertItem(int heap[], int size, int item){
    int parent, temp;
    size++;
    heap[size] = item;
    int i = size;
    while (i>1 and heap[i]>heap[parentIndex(i)]){
...
int main(){
    ...
    int size =10;
    int heap[size+1] = {0, 4, 10, 3, 5, 1, 6, 2, 11, 0};
    ...
    insertItem(heap, size, 8);
    cout<<"Item inserted to priority queue: ";
    for(int i=1; i<=size; i++){
        cout << heap[i] << " ";
    }
    cout << endl;
    ...
    cout<<"Extracted maximum from priority queue: ";
    for(int i=1; i<=size; i++){
        cout << heap[i] << " ";
    }
    cout << endl;
